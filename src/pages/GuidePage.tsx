...
import { getGuide, type Tip } from "@/data/dungeons";
...
const GuidePage = () => {
    ...
    return (
      <div className="min-h-dvh">
        ...
        <div className="mx-auto max-w-3xl px-6 py-32 text-center">
          ...
          <Link to="/" className="mt-6 inline-block text-primary underline">Back to Season 1 Pool</Link>
        </div>
      </div>
    );
  }
  ...
  const grouped = order
    .map((cat) => ({
      cat,
      tips: guide.tips.filter((t) => t.category === cat).sort((a, b) => a.order - b.order),
    }))
  ...
  return (
    <div className="relative min-h-dvh">
      ...
      <header className="relative overflow-hidden border-b border-border">
        <div className="absolute inset-0 bg-gradient-to-br from-void/10 via-transparent to-background" />
        ...
        <div className="relative mx-auto max-w-7xl px-6 py-20 md:py-28">
          ...
          <p className="mt-4 max-w-xl text-lg italic text-muted-foreground">{guide.tagline}</p>
        </div>
      </header>
        ...
        {guide.tips.length === 0 && (
          <div className="glass-panel rounded-2xl p-12 text-center">
            ...
            <p className="mt-3 text-muted-foreground">
              This guide is still being compiled. Check back as the season unfolds.
            </p>
          </div>
        )}
